...
const netValidation = Joi.object().keys({
    label: Joi.string().max(255).required().description("net name"),
    description: Joi.string().description("description, notes, etc."),
    tensors: Joi.array().items(Joi.array()).required().description("list of n-dimensional matricies")
});
...
const idValidation = Joi.object().keys({
    id: Joi.string().uuid().description("ID (UUID)")
});
