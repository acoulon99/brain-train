...
const netUploadValidation = Joi.object().keys({
    file: Joi.any().required().description("Zipped neural net files")
});
...
module.exports = [
    ...
    },
    {
        ...
        path: "/nets/{id}",
        config: {
            handler: handlers.uploadNet,
            payload: {
                output: 'stream',
                parse: true,
                allow: 'multipart/form-data'
            },
            description: "Upload a Net",
            notes: "Uploads a neural net",
            tags: ['api'],
            validate: {
                params: idValidation,
                headers: appKeyAuthValidation,
                payload: netUploadValidation
            }
        }
    },
    {
        method: "GET",
        path: "/nets/{id}/files.zip",
        config: {
            handler: handlers.getNetFile,
            description: "Get Net Files",
            notes: "Returns the files associated with this neural net",
            tags: ['api'],
            validate: {
                params: idValidation,
                headers: appKeyAuthValidation
